...
. From the application's **Releases** tab, click on the **Name** link

. Select the **Pipeline runs** tab

. Click on the name of the pipeline run to access details of the pipeline

. Select the **Security** tab to browse the results of the validation against the Conforma policy
...
== Exercise 8: Introduce and view a CVE vulnerability 

In this exercise, you will introduce a known CVE vulnerability in your repository and view this vulnerability in the vulnerability report generated by {RHTPAName}.

=== Modify your application source to introduce the vulnerability

. On your local machine, clone the sample GitLab repository:
+
[source,bash,role="execute",subs="attributes+"]
----
git clone {gitlab_url}/tsf/sample-component-golang.git
cd sample-component-golang
----

. Replace the root module files with vulnerable versions:
+
[source,bash,role="execute",subs="attributes+"]
----
...
. Create a push a commit:
+
[source,bash,role="execute",subs="attributes+"]
----
git commit -a -m "introducing vulnerability"
git push
----

Pushing the commit automatically triggers a Konflux build.

=== View the new report

. In the Konflux UI, . Navigate to your application → Components.
. Wait until the *Build status* becomes *Succeeded*.
. As described in Exercise 6, access the RHTPA UI and view the *Vulnerabilities* tab. Note the new CVE displaued in this tab.

=== Removing the vulnerability

. On your local machine, in the `sample-component-golang` directory, run the following commands:
+
[source,bash,role="execute",subs="attributes+"]
----
go get golang.org/x/text@latest
go mod tidy
git commit -a -m "removing vulnerability"
git push
----

After the build completes, you can review the vulnerabilities report to see that the CVE has been removed.
...
* ✓ You introduced a vulnerability and viewed it in the report
