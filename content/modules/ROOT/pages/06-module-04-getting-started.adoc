...
Now that {TSFShortName} is deployed and verified, you're ready to build your first secure application. In this module, you will use the Konflux UI to create an application, onboard a component from GitHub, and trigger a complete build-sign-release workflow.
...
For this workshop, you will use the Konflux sample component repository.

. Fork the sample repository to your GitHub organization:
+
link:https://github.com/konflux-ci/sample-component-golang[https://github.com/konflux-ci/sample-component-golang^]
+
Or use the repository directly (read-only).
...
+
----
https://github.com/<your-org>/sample-component-golang
----
+
Or use the sample directly:
+
----
...
. Go to your GitHub repository (if you forked it).

. Click **Pull requests**.

. You should see a pull request from Konflux adding `.tekton/` directory:
+
----
...
. Review the files but *do not merge yet*.

=== Verify

✓ Component created in Konflux +
✓ Pull request sent to GitHub repository +
✓ `.tekton/` directory added with pipeline definitions +
✓ Pipeline files visible in PR +

[TIP]
====
The `.tekton/` directory contains Tekton PipelineRun definitions that are triggered by GitHub/GitLab webhooks. These pipelines build, test, and sign your container images automatically.
====
...
. Create a basic release plan (run this from your {OCPCli} terminal):
+
[source,role="execute"]
----
cat <<EOF | oc apply -f -
...
. Verify the release plan was created:
+
[source,role="execute"]
----
oc get releaseplan my-first-app-plan -n default
----

=== Verify

✓ ReleasePlan created for your application +
...
. From your terminal, verify the image signature using cosign:
+
[source,role="execute"]
----
cosign tree quay.io/<your-org>/<component-name>:<tag>
----
+
Expected output:
+
----
📦 quay.io/your-org/sample-component-golang:abc123
├── 💾 attestations
│   └── <sha256 digest>
└── 🔐 signatures
    └── <sha256 digest>
...
* ✓ You triggered an automated build pipeline
...
* ✓ You viewed the automated release
...
Click *Next* for the workshop conclusion and troubleshooting guide.
