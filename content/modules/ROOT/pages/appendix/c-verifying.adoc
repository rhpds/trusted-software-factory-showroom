...
+
----
cert-manager-operator          Active   10m
konflux-ui                     Active   8m
openshift-storage              Active   12m
rhbk-operator                  Active   11m
rhtpa-operator                 Active   9m
tsf                            Active   15m
tsf-tpa                        Active   9m
----
...
----
NAME                           READY   STATUS    RESTARTS   AGE
konflux-ui-xxxxx-xxxxx         1/1     Running   0          8m
----
...
----
NAME         HOST/PORT
konflux-ui   konflux-ui-konflux-ui.apps.cluster-abc123.ocpv00.rhdp.net
----
...
[source,bash]
----
KONFLUX_URL=$(oc get route konflux-ui -n konflux-ui -o jsonpath='{.spec.host}')
echo "Konflux UI: https://$KONFLUX_URL"
----

=== Get RHTPA UI route

. Find the {RHTPAName} UI route.
+
[source,role="execute"]
----
...
----
NAME       HOST/PORT
rhtpa-ui   rhtpa-ui-tsf-tpa.apps.cluster-abc123.ocpv00.rhdp.net
----
...
[source,bash]
----
RHTPA_URL=$(oc get route rhtpa-ui -n tsf-tpa -o jsonpath='{.spec.host}')
echo "RHTPA UI: https://$RHTPA_URL"
----
...
** Navigation menu with Applications, Releases, etc.
...
[source,role="execute"]
----
oc get pods -n tsf | grep -E '(fulcio|rekor|tuf)'
----
+
You should see:
